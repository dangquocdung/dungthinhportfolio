   ...
   public function formview()
	...
	protected function addpagedata(Request $request)
    {
	   ...
	   $imgtype = $settings[0]->image_type;
		...
		DB::insert('insert into pages (page_title, post_slug,  page_desc, photo) values (?, ?, ?, ?)', [$page_title,$this->clean($page_title),$page_desc,$namef]);
   ...
   protected function pagedata(Request $request)
    {
	   ...
	   $imgtype = $settings[0]->image_type;
		...
		DB::update('update pages set page_title="'.$page_title.'",post_slug="'.$this->clean($page_title).'",page_desc="'.$page_desc.'",photo="'.$namef.'" where page_id = ?', [$page_id]);
